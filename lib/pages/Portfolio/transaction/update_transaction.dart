...
import 'package:flutter_application_1/network/request_put_transaction.dart';
...
class UpDateTransaction extends StatefulWidget {
  ...
  @override
  State<UpDateTransaction> createState() => _MyWidgetState();
}

class _MyWidgetState extends State<UpDateTransaction> {
  ...
  void initState() {
    super.initState();
    selectionDateTime = widget.time;
    widget.type == "Mua"
        ? selections = [true, false]
        : selections = [false, true];
    selectionType = widget.type;
    _controllerPurchasePrice.text = widget.price.toString();
    purchasePrice = widget.price.toString();
    _controllerTotal.text=widget.soTien.toString();
     total = widget.soTien.toString();
    checkInputValue();
  }
  ...
  Future upDateTransaction() async {
    ...
    NetworkRequestUpDateTracsaction networkRequestUpDateTracsaction =
        NetworkRequestUpDateTracsaction(jsonBody: transaction,idTransaction:widget.idTransaction );
    await networkRequestUpDateTracsaction.fetchPosts();
  ...
  void checkInputValue() {
    if (widget.nameCoin.isNotEmpty == true &&
